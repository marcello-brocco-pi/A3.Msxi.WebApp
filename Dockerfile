...
# Usa un'immagine più leggera per servire l'app (Nginx)
...
COPY --from=builder /app/dist/msxiwebapp/browser /usr/share/nginx/html
...
RUN rm /etc/nginx/nginx.conf && \
    ...
    http { \
        ...
        server { \
            ...
            index index.html; \
            \
            # 🔹 Serve i file statici, incluso .mjs con il MIME corretto \
            location ~* \.mjs$ { \
                root /usr/share/nginx/html; \
            ...
            \
            # 🔹 Angular SPA routing senza interferire con i file statici \
            location / { \
